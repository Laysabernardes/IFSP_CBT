    ...
    class Vendedores
    {
        ...
        public bool DelVendedor(int id)
        {
            for (int i = 0; i < Qtde; i++)
            {
                if (OsVendedores[i].Id == id)
                {
                    ...
                    {
                        Console.WriteLine(
                            "Não é possível excluir um vendedor com vendas registradas."
                        );
                        return false;
                    }
                }
        ...
        public void ListarVendedores()
        {
            ...
            for (int i = 0; i < Qtde; i++)
            {
                ...
                {
                    Console.WriteLine(
                        $"ID: {OsVendedores[i].Id}, Nome: {OsVendedores[i].Nome}, Valor Total de Vendas: {OsVendedores[i].ValorVendas():C}, Comissão: {OsVendedores[i].ValorComissao():C}"
                    );
                    totalVendas += OsVendedores[i].ValorVendas();
                    totalComissao += OsVendedores[i].ValorComissao();
                }
